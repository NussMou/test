...
/////////////////////////////////////////////////////////////////////
    ...
    int blocksize_sq = blockSize * blockSize;
    int *globalx = (int *)malloc(blockSize * sizeof(int));
    int *globaly = (int *)malloc(blockSize * sizeof(int));

    alignas(32) __m256i simd_globalx;
    alignas(32) __m256i simd_globaly;
    alignas(32) __m256i simd_eight;
    simd_eight = _mm256_set_epi32(8, 8, 8, 8, 8, 8, 8, 8);

    for (Ii = 0; Ii < nBlockRows; Ii++)
    {
        i = 1;
        globalx[0] = (Ii + Istart) * blockSize;
        if(blockSize >= 8){
            simd_globalx = _mm256_set_epi32(globalx[0] + 7, globalx[0] + 6, globalx[0] + 5, globalx[0] + 4, globalx[0] + 3, globalx[0] + 2, globalx[0] + 1, globalx[0]);
            _mm256_store_si256((__m256i*)&globalx[0], simd_globalx);
            for (i = 8; i < blockSize - 8; i += 8)
            {
                simd_globalx = _mm256_add_epi32(simd_globalx, simd_eight);
                _mm256_store_si256((__m256i*)&globalx[i], simd_globalx);
            }
        } 
        
        for (; i < blockSize; i++){
            globalx[i] = globalx[i - 1] + 1;
        }

        for (int i = rpt[Ii]; i < rpt[Ii + 1]; i++)
        {
            globaly[0] = cpt[i] * blockSize;
            for (int j = 1; j < blockSize; j++)
            {
                globaly[j] = globaly[j - 1] + 1;
            }
